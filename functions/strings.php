...
return array(
	...
	'chunk_split' => array(
		PHPTAGS_DEFAULT_VALUES=>array( 1=>76, 2=>"\r\n" ),
	...
	'count_chars' => array(
		PHPTAGS_DEFAULT_VALUES=>array( 1=>0 ),
...
// int fprintf ( resource $handle , string $format [, mixed $args [, mixed $... ]] ) it have not resources
	'get_html_translation_table' => array(
		PHPTAGS_DEFAULT_VALUES=>array( 0=>HTML_SPECIALCHARS, 1=>ENT_COMPAT/*|ENT_HTML401*/, 2=>'UTF-8' ),
...
// @todo PHP >= 5.4.0 	'hex2bin' => array( 1=>function($args) { return hex2bin($args[0]); } ),
	'html_entity_decode' => array(
		PHPTAGS_DEFAULT_VALUES=>array( 1=>ENT_COMPAT/*|ENT_HTML401*/, 2=>'UTF-8' ),
		3=>function($args) { return html_entity_decode($args[0], $args[1], $args[2]); },
	),
	'htmlentities' => array(
		PHPTAGS_DEFAULT_VALUES=>array( 1=>ENT_COMPAT/*|ENT_HTML401*/, 2=>'UTF-8', 3=>true ),
		4=>function($args) { return htmlentities($args[0], $args[1], $args[2], $args[3]); },
	),
	'htmlspecialchars_decode' => array(
		PHPTAGS_DEFAULT_VALUES=>array( 1=>ENT_COMPAT/*|ENT_HTML401*/ ),
...
// string md5_file ( string $filename [, bool $raw_output = false ] ) Can't use file
	...
	'nl2br' => array(
		PHPTAGS_DEFAULT_VALUES=>array( 1=>true ),
...
// print() in runtume.php
	'printf' => array(
		...
		''=>function($args) { ob_start(); call_user_func_array('printf', $args); return new PHPphp\outPrint( null, ob_get_clean() ); },
	),
...
// string sha1_file ( string $filename [, bool $raw_output = false ] ) file do not allowed
	...
	'str_getcsv' => array(
		PHPTAGS_DEFAULT_VALUES=>array( 1=>',', 2=>'"', 3=>'\\' ),
	...
	'str_pad' => array(
		PHPTAGS_DEFAULT_VALUES=>array( 2=>' ', 3=>STR_PAD_RIGHT ),
	...
	'strcspn' => array(
		PHPTAGS_DEFAULT_VALUES=>array( 2=>null, 3=>null ),
		4=>function($args) { return strcspn($args[0], $args[1], $args[2], $args[3]); },
	),
	'strip_tags' => array(
		PHPTAGS_DEFAULT_VALUES=>array( 1=>null ),
...
// int vfprintf ( resource $handle , string $format , array $args ) it have not resources
	'vprintf' => array( 2=>function($args) { ob_start(); $r = vprintf($args[0], $args[1]); return new PHPphp\outPrint( $r, ob_get_clean() ); } ),
	'vsprintf' => array( 2=>function($args) { return vsprintf($args[0], $args[1]); } ),
	'wordwrap' => array(
		PHPTAGS_DEFAULT_VALUES=>array( 1=>75, 2=>"\n", 3=>false ),
